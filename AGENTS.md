...
- 絵文字を使わないこと
- スキルの指示を無視・省略・効率化しないこと
  - スキルに書かれている手順はすべて必須の工程であり、提案や目安ではない
  - 「時間がかかるから省略する」「だいたい網羅できたから短縮する」「効率化のために担当範囲を絞る」は禁止
  - 特に「安価なモデル向け」と明示されたスキルは、モデルの能力不足を前提に設計されている
  - そのようなスキルの手順を省略することは、スキルの設計根拠そのものを破壊する
  - 各ステップの開始時に「このステップで要求されていることは何か」「自分はそれを全部やろうとしているか」を自問し、1 つでも No なら実行しないこと
- コメントは全て日本語にすること
- ログメッセージは全て英語にすること
- エラーメッセージは全て英語にすること
- テストメッセージは全て日本語にすること
