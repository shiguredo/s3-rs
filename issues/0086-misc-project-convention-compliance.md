...
AGENTS.md で定められた規約違反を解消する。機能バグではないが、リリース品質と CI 一貫性に影響する。
...
| 項目 | 場所 |
|------|------|
| `#[allow(...)]` 使用 | `src/api/mod.rs` (3 箇所), `examples/s3cli/` (6 箇所) |
...
`#[allow]` を `#[expect]` に変更する。対象: `src/api/mod.rs` (3 箇所), `examples/s3cli/` (6 箇所)。
...
- HttpDate の矛盾を整理する: 派生元ブランチとの最終差分のみを残し、L52 ADD（追加）は削除、L96 CHANGE（廃止）と L132 FIX（修正）は残す
- shiguredo_http11 の中間更新記述を統合する: L137（2026.2 → 2026.5）と L141 の http11 部分（2026.1 → 2026.2）を「2026.1 → 2026.5」に統合する。L141 の `hmac` / `md-5` / `sha1` / `sha2` 更新記述は別途残す

### 依存バージョンの修正

`shiguredo_http11 = "2026.5.0"` を `shiguredo_http11 = "2026.5"` に変更する。`tokio = "1"` は Cargo のセマンティックバージョニングで `>=1.0.0` を意味するため変更不要。

### CI の clippy 修正

`cargo clippy --workspace -- -D warnings` を `cargo clippy --workspace --all-targets -- -D warnings` に変更する。`--all-features` は排他 features のため使用不可。

### 陳腐化コメントの削除

`types.rs` 112, 457 行目の解決済み issue 参照 TODO を削除する。
...
- `cargo clippy --workspace --all-targets -- -D warnings` が CI で実行される
